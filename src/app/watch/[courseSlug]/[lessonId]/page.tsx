...
import { ArrowLeft, Play, CheckCircle2, FileText, HelpCircle, Download, Send, Clock, Award, ShieldCheck, ChevronRight } from 'lucide-react';
...
export default function WatchLessonPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0B] text-white flex flex-col pt-16">
      {/* Top Navigation Bar */}
      <header className="h-14 bg-[#141414] border-b border-white/10 px-4 sm:px-6 flex items-center justify-between text-xs z-20">
        <div className="flex items-center gap-3">
          ...
          <div className="truncate max-w-md">
            ...
            <span className="text-red-400 font-medium">{currentLesson.title}</span>
          </div>
        </div>
      ...
      {/* Main Full-Width Immersive Theater Grid */}
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Left: Main Media & Content Hub */}
        <div className="flex-1 flex flex-col overflow-y-auto no-scrollbar">
          {/* Theater Mode Video Container */}
          <div className="relative aspect-video w-full bg-black flex items-center justify-center border-b border-white/10 shadow-2xl">
          ...
          <div className="bg-[#141414] border-b border-white/10 px-6 flex gap-6 text-xs font-semibold">
            <button
              ...
              className={`py-3.5 border-b-2 transition ${
                activeTab === 'overview' ? 'border-[#E50914] text-white' : 'border-transparent text-[#B3B3B3] hover:text-white'
              }`}
            >
              Overview & Resources
            </button>
            <button
              ...
              className={`py-3.5 border-b-2 transition ${
                activeTab === 'qa' ? 'border-[#E50914] text-white' : 'border-transparent text-[#B3B3B3] hover:text-white'
              }`}
            >
            ...
            {activeTab === 'overview' && (
              <div className="space-y-4 text-xs text-[#B3B3B3]">
                ...
                <p className="leading-relaxed">
                  Welcome to {currentLesson.title}! In this module, you will learn production architectural practices, error handling, and server deployment workflows.
                </p>
              </div>
            )}
              ...
              <div className="space-y-6">
                <form onSubmit={handlePostQuestion} className="space-y-2">
                  <textarea
                    ...
                    className="w-full bg-[#141414] border border-white/15 rounded-md p-3 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-[#E50914]"
                  />
                  <button
                    type="submit"
                    className="bg-[#E50914] hover:bg-[#b80710] text-white font-bold text-xs px-4 py-2 rounded-md transition flex items-center gap-1.5"
                  >
        ...
        {/* Right Collapsible Sidebar: Curriculum Tree */}
        <div className="w-full lg:w-96 bg-[#141414] border-l border-white/10 flex flex-col flex-shrink-0">
                  ...
                  {section.lessons.map(lesson => {
                    ...
                    return (
                      <Link
                        ...
                        className={`p-3 pl-4 flex items-center justify-between text-xs transition ${
                          isCurrent
                            ? 'bg-[#E50914]/20 text-white font-bold border-l-4 border-[#E50914]'
